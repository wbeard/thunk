...
from src.thunk.types import SearchResult, Document
...
class MockGeminiLLM:
    ...
    def __init__(self):
        self.plan_research = Mock(return_value=MockResearchPlan([
            "Background research", 
            "Current developments", 
            "Future prospects"
        ]))
        self.generate_search_queries = Mock(return_value=[
            "test query 1", 
            "test query 2"
        ])
        self.assess_research_completeness = Mock(return_value=(True, None))
        self.evaluate_source_relevance = Mock(return_value=("HIGHLY_RELEVANT", 0.8, "Good source"))
...
class MockSearchResult:
    ...
    def __init__(self, title: str = "Test Article", url: str = "https://test.com", rank: int = 1):
        self.title = title
        self.url = url
        self.rank = rank
...
class MockDocument:
    ...
    def __init__(self, id: str = "doc_1", title: str = "Test Doc", url: str = "https://test.com"):
        self.id = id
        self.title = title
        self.url = url
...
class TestDeepResearchAgent:
    ...
    def test_synthesize_report_from_documents(self, mock_agent, sample_documents):
        ...
        with patch.object(mock_agent.rag_engine, 'get_corpus_summary', return_value={"file_count": 5}), \
             patch.object(mock_agent.rag_engine, 'generate_with_rag', return_value="RAG report"), \
             patch.object(mock_agent, '_generate_references', return_value="## References\n[1] Test ref"):
    ...
    async def test_search_and_analyze(self, mock_agent):
        ...
        with patch.object(mock_agent, '_fetch_and_process_content', return_value=MockDocument()) as mock_fetch:
    ...
    async def test_execute_focused_research_max_iterations(self, mock_agent):
        ...
        result = await mock_agent._execute_focused_research(
            "test query", "focus area", sample_docs, 3, 3  # Already at max
        )
    ...
    async def test_execute_focused_research_recursion(self, mock_agent):
        ...
        with patch.object(mock_agent, '_search_and_analyze_with_semaphore', return_value=[MockDocument()]):
            result = await mock_agent._execute_focused_research(
                "test query", "focus area", sample_docs, 0, 3
            )
    ...
    async def test_research_with_context_integration(self, mock_agent):
        ...
        with patch.object(mock_agent, '_search_and_analyze_with_semaphore', return_value=[MockDocument()]) as mock_search, \
             patch.object(mock_agent, '_synthesize_report_from_documents', return_value="Final report") as mock_synthesize:
            
            result = await mock_agent.research_with_context("test query", "test context")
    ...
    async def test_research_with_context_focused_research_integration(self, mock_agent):
        ...
        with patch.object(mock_agent, '_search_and_analyze_with_semaphore', return_value=[MockDocument()]) as mock_search, \
             patch.object(mock_agent, '_execute_focused_research', return_value=[MockDocument(), MockDocument()]) as mock_focused, \
             patch.object(mock_agent, '_synthesize_report_from_documents', return_value="Final report") as mock_synthesize:
    ...
    async def test_research_with_retry(self, mock_agent):
        ...
        mock_agent.research_with_context = AsyncMock(side_effect=[
    ...
    async def test_research_with_retry_exhausted(self, mock_agent):
        ...
        mock_agent.research_with_context = AsyncMock(side_effect=Exception("Always fails"))
    ...
    async def test_regenerate_summary(self, mock_agent, sample_documents):
        ...
        with patch.object(mock_agent, '_synthesize_report_from_documents', return_value="Regenerated report") as mock_synthesize:
...
class TestDeepResearchAgentIntegration:
    ...
    async def test_full_research_flow_mock(self, mock_agent):
        ...
        with patch.object(mock_agent, '_fetch_and_process_content', return_value=MockDocument()) as mock_fetch, \
             patch.object(mock_agent, '_synthesize_report_from_documents', return_value="Complete report") as mock_synthesize:
