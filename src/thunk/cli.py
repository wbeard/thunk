...
from .search.web_search_agent import WebSearchAgent
from .search.arxiv_search_agent import ArxivSearchAgent
...
class CLIEventSubscriber:
    ...
    def subscribe_to_agent(self, agent: DeepResearchAgent):
        ...
        agent.subscribe(
            "research_completeness_check", self.on_research_completeness_check
        )
        ...
        agent.subscribe(
            "regeneration_documents_found", self.on_regeneration_documents_found
        )
    ...
    def on_research_step_after(self, step_num: int, research_step: str, doc_count: int):
        self._print(
            f"Status: Step {step_num} completed ({doc_count} documents collected)"
        )
    ...
    def on_regeneration_documents_after(
        self, vertex_count: int, local_count: int, unique_count: int
    ):
        self._debug_print(
            f"Found {vertex_count} Vertex AI docs, {local_count} local docs, {unique_count} unique"
        )
    ...
    def on_source_evaluation_after(
        ...
        if accepted:
            ...
            self._debug_print(
                f"Rejected: {title[:50]}... (score: {score:.2f}) - {reason}"
            )
    ...
    def on_content_fetch_after(
        ...
        if success:
            ...
            self._print("Status: Content fetch failed")
    ...
    def on_document_storage_after(
        self, doc_id: str, storage_type: str, success: bool, error: Exception = None
    ):
        if success:
    ...
    def on_focused_research_before(
        self, focus_area: str, iteration: int, max_iterations: int
    ):
        self._print(
            f"Status: Starting focused research on '{focus_area[:40]}...' (iteration {iteration + 1}/{max_iterations})"
        )
    ...
    def on_research_completeness_check(
        self, is_sufficient: bool, next_focus: str = None
    ):
        pass
...
class BasicResearchCLI:
    ...
    def __init__(self, corpus_display_name: str = None, search_provider: str = "web"):
        ...
        self.config = ResearchConfig(
            corpus_display_name=corpus_display_name, search_provider=search_provider
        )
    ...
    def setup_agent(
        ...
        try:
            ...
            self.logger_subscriber = ResearchEventLogger(debug_mode=debug_mode)
    ...
    def gather_clarifications(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            for i, question in enumerate(questions, 1):
                if not (
                    self.logger_subscriber and hasattr(self.logger_subscriber, "logger")
                ):
                    print(f"\n{i}. {question}")
                ...
                while True:
                    ...
                    if response:
                        if self.logger_subscriber and hasattr(
                            self.logger_subscriber, "logger"
                        ):
                            self.logger_subscriber.logger.add_clarification_response(
                                question, response
                            )
    ...
    async def conduct_research(
        ...
        try:
            ...
            print(f"Local documents collected: {local_info.get('local_documents', 0)}")
    ...
    async def regenerate_summary(
        ...
        try:
            ...
            print(f"Vertex AI RAG documents: {vertex_info.get('file_count', 0)}")
            print(f"Local documents available: {local_info.get('local_documents', 0)}")
    ...
    def delete_corpus(self) -> bool:
        ...
        try:
            ...
            print(
                "This action cannot be undone and will permanently remove all documents from the corpus."
            )
    ...
    async def interactive_mode(self, debug_mode: bool = False):
        ...
        try:
            ...
            print(
                "🗄️ Documents remain stored in Vertex AI RAG corpus for future sessions."
            )
        except KeyboardInterrupt:
...
def main():
    ...
    parser.add_argument(
        "--delete-corpus",
        "-D",
        help="Delete the specified corpus (requires corpus name)",
    )
    ...
    cli = BasicResearchCLI(
        corpus_display_name=args.corpus, search_provider=args.search_provider
    )
    ...
    if args.delete_corpus:
        if not args.corpus:
            print(
                "Error: --delete-corpus requires --corpus to specify which corpus to delete"
            )
            return 1

        if args.delete_corpus != args.corpus:
            print(
                f"Error: --delete-corpus value '{args.delete_corpus}' must match --corpus value '{args.corpus}'"
            )
            return 1
